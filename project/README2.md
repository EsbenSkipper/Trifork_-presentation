...
| ---------- | ----------- | ---- |
| **Router**| **You** — classify the intent, then a hand-written `if/elif` dispatches | [`Old_LLM_BOT/main.py`](Old_LLM_BOT/main.py) |
| **Tool Calling**| **The model** — you hand it tools and it picks which to call | [`LLM_BOT/main_tools.py`](LLM_BOT/main_tools.py) |
...
flowchart TD
    ...
    I -->|WE branch on intent| SQ["SpendQuery · LLM<br/>relational_filters: categories ['Groceries'],<br/>date_from '2025-01-01', date_to '2025-12-31'<br/>aggregation: type 'sum'"]
...
**🔵 marks an LLM API call.** Every turn starts with **call #1 · classify intent**;
...
**Spend** — the heaviest path (up to **3** API calls)
...
| grounded explanation text             | ✅ optional               | nicer wording — but must forbid hallucination |
| parse free-form dispute reason         | ✅ optional               | handles free text — but still whitelist the 4 reasons |
| spend-result wording                  | ✅ optional               | nicer — but risks mismatching the numbers |
...
- **✅ optional** → you *can* push it to the model to make it nicer, but validate in Python.
...
The same flows the Mermaid diagrams above show, as plain text — handy for reading
